    ...
    <div className="px-3 py-2.5 rounded-xl bg-surface-container-high border border-outline-variant/50 text-xs shadow-xl whitespace-nowrap">
      ...
      {port != null && (
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-on-surface font-semibold font-mono ml-2">{fmtPct(port)}</span>
        </div>
      )}
      {ibov != null && (
        <div className="flex items-center gap-2">
          ...
          <span className="text-on-surface font-semibold font-mono ml-2">{fmtPct(ibov)}</span>
        </div>
      )}
    </div>
  );
      ...
      {!!data?.series.length && (
        <>
          <div className="grid grid-cols-3 gap-4 mb-5">
            <div>
              <div className="flex items-center gap-1.5 mb-1">
                <span className="w-2 h-2 rounded-full bg-tertiary" />
                <p className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold">Carteira</p>
              </div>
              <p className={cn('font-headline font-extrabold tracking-tight text-2xl', winning ? 'text-tertiary' : 'text-error')}>
                {fmtPct(port)}
              </p>
            </div>
            <div>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <p className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold">IBOV</p>
              </div>
              <p className={cn('font-headline font-extrabold tracking-tight text-2xl', ibov >= 0 ? 'text-tertiary' : 'text-error')}>
                {fmtPct(ibov)}
              </p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold mb-1">Δ Alpha</p>
              <p className={cn('font-headline font-extrabold tracking-tight text-2xl', winning ? 'text-tertiary' : 'text-error')}>
                {fmtPct(delta)}
              </p>
            </div>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData} margin={{ top: 4, right: 8, left: -8, bottom: 0 }}>
              ...
              <Line
                ...
                strokeWidth={2.4}
                dot={false}
                activeDot={{ r: 4, fill: 'rgb(52 211 153)', stroke: 'rgb(13 27 42)', strokeWidth: 2 }}
              />
              <Line
                type="monotone"
                dataKey="ibovPercent"
          ...
          <div className={cn(
            ...
            winning ? 'bg-tertiary/10 text-tertiary' : 'bg-error/10 text-error'
          )}>
