...
function ConcentrationRow({ h, maxPct }: { h: ConcentrationHolding; maxPct: number }) {
  ...
  return (
    <tr className={cn('border-b border-outline-variant/10 last:border-b-0 transition-colors', rowBg)}>
      <td className="px-6 md:px-7 py-3">
        <div className="flex items-center gap-3">
          <div className={cn('w-8 h-8 rounded-lg text-[10px] font-mono font-bold flex items-center justify-center shrink-0', chipCls)}>
            {h.ticker.slice(0, 4)}
          </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {(overConcentrated || overWeight) && (
                <span className={cn(
                  ...
                  overConcentrated ? 'bg-error/12 text-error' : 'bg-amber-400/12 text-amber-400'
                )}>
      ...
      <td className="px-3 py-3 w-52">
        <div className="flex items-center gap-2.5">
          <div className="flex-1 h-2 rounded-full bg-surface-container-high overflow-hidden">
            <div
              className={cn('h-full rounded-full transition-all duration-500', barFill)}
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            {overWeightCount > 0 && (
              <div className="flex items-center gap-1 text-amber-400">
                ...
                <span className="text-[11px] font-medium">{overWeightCount} sobrepeso</span>
              </div>
            )}
            {overConcCount > 0 && (
              <div className="flex items-center gap-1 text-error">
                <AlertTriangle size={10} className="shrink-0" />
                <span className="text-[11px] font-medium">{overConcCount} super concentrado</span>
              </div>
            )}
          </div>
        )}
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10 shadow-[0_1px_0_0_hsl(var(--outline-variant)/0.2)]">
              <tr className="text-[10px] uppercase tracking-widest text-on-surface-variant/60 font-extrabold bg-surface-container-lowest">
                ...
                <th className="text-right font-extrabold py-3 px-3 w-52">% Book</th>
                <th className="text-right font-extrabold py-3 px-3">Clientes</th>
