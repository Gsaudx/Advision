...
function ExpiryRow({ w, max, index }: { w: OptionsExpiryWindow; max: number; index: number }) {
    ...
    <div className="group">
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-semibold text-on-surface">{w.label}</span>
          <span className="text-[11px] text-on-surface-variant">{w.count} contratos</span>
        </div>
        ...
        <>
          ...
          {criticalWindow && criticalWindow.count > 0 && (
            <div className="mt-5 pt-4 border-t border-outline-variant/20 flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[11px] text-on-surface-variant">requerem rolagem ou exercício</span>
            </div>
          )}
        </>
      )}
