...
/**
 * Visão de métricas do cliente — reaproveita os widgets de analytics, escopados
 * pelo backend às carteiras do próprio cliente. Omite widgets exclusivos do
 * assessor (ranking de clientes e ações pendentes).
 */
export function ClientAnalyticsPage() {
  ...
  const baseParams: AnalyticsBaseParams = {
    mode,
    walletId: walletId ?? undefined,
  };
  const periodParams: AnalyticsPeriodParams = {
    ...baseParams,
    period,
    customFrom: safeCustomFrom,
    customTo: safeCustomTo,
  };
  const evolutionParams: AnalyticsEvolutionParams = {
    mode,
    walletId: walletId ?? undefined,
    period,
    customFrom: safeCustomFrom,
  ...
  return (
    <div className="max-w-[1440px] mx-auto pb-12 space-y-6">
      ...
      {/* ── Concentração de Ativos (100%) ── */}
      <AssetConcentration params={baseParams} />

      {/* ── Melhores/Piores (70%) + Proventos (30%) ── */}
      <div className="flex gap-6 items-stretch">
        <div className="flex-[7] min-w-0 flex flex-col">
